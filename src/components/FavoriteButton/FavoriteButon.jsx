...
export const FavoriteButon = ({ className, id }) => {  // кнопка Лайк, id-id товара
   ...
   const [hover, setHover] = useState(false);  // меняем состояние у поля hover
   const dispatch = useDispatch();
   const favoriteList = useSelector((state)=> state.favorite.favoriteList); 

   const isFavorite = favoriteList.includes(id);         // если id в массиве favoriteList
   ...
   return (
      // onMouseEnter -событие при навдении мыши, при нажатии вызвовется handleFavoriteClick,  onMouseLeave-собыие когда убрали курсор  :
