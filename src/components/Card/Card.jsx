...
import { Navigation, Thumbs } from "swiper/modules";  //  Thumb нужен для связвания двух слайдеров(маленькго и большого)
...
export const Card = () => {
   ...
   useEffect(() => {  // в компоненте нельзя вызвать асинхронную функию(fetchСategories), а внутри useEffect() можно 
    ...
    }, [ dispatch, productId ]);  // коллбэк вызывается когда  меняется data.  [] - массив зависимостей. Сюда заносятся поля, котрые используютя  вколлбэке
      ...
      <section className={s.card}> 
         <Container className={s.container}>       {/* Грид контегнйер  */}
            ...
            <div className={s.picture}>            {/* Грид элемент  */}
               ...
               {/* нижний слайдер: */}
               <div className={s.sliderMain}>
                  <Swiper modules={[Thumbs]} onSwiper={setThumbsSwiper} watchSlidesProgress={true} spaceBetween={14} slidesPerView={4} freeMode={true}>       {/* собтие onSwiper происходит когда swiper инициализируется, и вызывается setThumbsSwiper() */}
                     <SwiperSlide> 
                        <img className={s.image} src="/img/cart.jpg" alt="" />  {/*  картинки берем из папки public */}
                     </SwiperSlide> 
            ...
            <div className={s.info}> {/* Грид элемент  */}
               <p className={s.price}> 500 </p>          {/*  {'5000'.localeString()}&nbsp;Р  */}
               <p className={s.article}> арт. 84348945757 </p>
                ...
                <div className={s.characteristics}>      {/* ДОДЕЛАТЬ */}
                     ...
                     <table className={s.table}>
                        <tr className={s.row}>
                           <td className={s.field}> Коллекция </td>
                           <td className={s.value}> Мирсаж </td>
                        </tr>
                     </table>
