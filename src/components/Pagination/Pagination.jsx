...
export const Pagination = ({ pagination }) => {  // pagination: { currentPage: 1, totalPages: 2, totalProducts: 19, limit: 12 }

   const { currentPage, totalPages, totalProducts, limit } = pagination; 

   const location = useLocation();        // хук, позволяет опреелить по какому урлу сейчас находимся
   
   const [ searchParam ] = useSearchParams();                          // хук, нужен чтобы получить search парамтеры(page, category) из урла
   
   const currentPageNumber = parseInt(searchParam.get('page')) || currentPage;               // parseInt() переводит из сроки в число, получить значение search-параметра  page
   ...
   const createPageUrl = (pageNumber) => {
      ...
      return `${location.pathname}?${newSearchParams.toString()}`;  // формируем урл, пример  /favorite?page=2
   }
   ...
   const prevPageUrl = prevPageNumber > 0 ? createPageUrl(prevPageNumber) : '';           //  favorite?page=1
                                          // либо вместо totalPages так: Math.ceil(totalProducts/limit)
   const nextPageUrl = nextPageNumber <= totalPages ? createPageUrl(nextPageNumber) : '';       //   favorite?page=2
